document.addEventListener("DOMContentLoaded", function () {
  ...
  const triggerInvalidAmountFeedback = () => {
    ...
    if (navigator.vibrate) {
      // Trigger a short, 100-millisecond vibration to signal an error.
      navigator.vibrate(100);
    }
  };

  /**
   * An advanced event handler that intercepts the click on the recharge button.
   * It performs a final validation check before allowing the action to proceed.
